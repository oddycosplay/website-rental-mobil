<x-guest-layout>
    <x-authentication-card>
        <h2 class="text-2xl font-bold text-center text-white mb-8">Reset Password</h2>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="space-y-6">
            ...
            <div>
                ...
                <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password"
                    class="block w-full bg-slate-900/50 border border-slate-700 text-white rounded-xl focus:ring-gold focus:border-gold placeholder-slate-500 transition-colors shadow-inner" placeholder="Ulangi password baru" />
            </div>
